...
|fs.cosn.socket.error.maxRetries | 针对 Socket 异常（如网络抖动、连接重置等）的最大重试次数 | 5 | 否 |
|fs.cosn.use.short.connection | 是否使用短连接（即每次请求后关闭连接）。默认使用长连接以提升性能，仅在特殊网络环境下建议开启 | false | 否 |
|fs.cosn.connection.request.timeout | 从连接池中获取连接的超时时间，单位为毫秒。-1 表示无限等待 | -1 | 否 |
|fs.cosn.connection.timeout | 与 COS 服务端建立 TCP 连接的超时时间，单位为毫秒 | 10000 | 否 |
|fs.cosn.idle.connection.alive | 连接池中空闲连接的最大存活时间，单位为毫秒。超过该时间的空闲连接将被回收 | 60000 | 否 |
|fs.cosn.client.socket.timeoutsec | 客户端 Socket 读取数据的超时时间，单位为秒。超过该时间未收到数据则抛出 SocketTimeoutException | 30 | 否 |
|fs.cosn.client.use.request.timeout | 是否启用 COS 客户端的请求超时机制。开启后，`fs.cosn.client.request.timeout` 和 `fs.cosn.client.request.timeout.thread.size` 配置才会生效 | false | 否 |
|fs.cosn.client.request.timeout | 单次 COS 请求的最大超时时间，单位为毫秒。需先开启 `fs.cosn.client.use.request.timeout` | 60000 | 否 |
|fs.cosn.client.request.timeout.thread.size | 请求超时监控线程池的线程数。需先开启 `fs.cosn.client.use.request.timeout` | 10 | 否 |
...
|fs.cosn.traffic.limit | 上传下载带宽的控制选项，819200 ~ 838860800，单位为bits/s。默认值为-1，表示不限制。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |                                                                                                                                                                                          -1                                                                                                                                                                                          | 否 |
 | fs.cosn.read.page.cache.enabled | 是否开启预读页面的淘汰缓存，开启以后，对于每次预读的页面都会尝试缓存在临时磁盘目录中（由 fs.cosn.read.page.cache.dir 指定，默认在 /tmp/hadoop_cos/page_cache 下），这个选项对于顺序读和随机读混合的场景下，能有效提升随机读的性能表现。| false | 否 |
