...
|fs.cosn.server-side-encryption-algorithm | 配置COS服务端加密算法，支持SSE_C和SSE_COS，默认为空，不加密| 无 | 否|
|fs.cosn.server-side-encryption.key | 当开启COS_C服务端加密算法时，必须配置SSE_C的密钥，默认为空，不加密| 无 | 否|
