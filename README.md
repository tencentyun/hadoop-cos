...
|fs.cosn.buffer.dir                | 请设置一个实际存在的目录，运行过程中产生的临时文件会暂时放于此处。如果buffer指定了disk类型，则该目录需要预留至少fs.cosn.upload.buffer.size大小的空间| /tmp/hadoop_cos| 否|
...
命令格式为：`hadoop fs -ls cosn://bucket 路径`或`hadoop fs -ls /`(配置了fs.defaultFS选项为 cosn://bucket 后) ，下例中以名称为 hdfs-test-1252681929 的 bucket 为例，可在其后面加上具体路径。
