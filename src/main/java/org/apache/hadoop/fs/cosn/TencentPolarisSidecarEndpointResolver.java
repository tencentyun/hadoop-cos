...
public class TencentPolarisSidecarEndpointResolver implements RetryableEndpointResolver, HandlerAfterProcess {
    private static final Logger LOG = LoggerFactory.getLogger(TencentPolarisSidecarEndpointResolver.class);
    ...
    public TencentPolarisSidecarEndpointResolver(TencentPolarisSidecarClient polarisClient, String namespace, String service) {
    ...
    public void handle(int status, long costTime) {
        if (null != this.lastHost && this.lastPort > 0) {
            ...
            item.retStatus = status == 0 ? RetStatus.RetSuccess.ordinal() : RetStatus.RetFail.ordinal();
            item.retCode = status;
    ...
    @Override
    public TencentPolarisSidecarEndpointResolver withMaxRetries(int maxRetries) {
